...
def main(args):
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                # kp2dX['kp_2d'] = convert_crop_coords_to_orig_img(
                #     bbox=bbox,
                #     keypoints=output['kp_2d'],
                #     crop_size=224,
                # )


                # cv_keypoints =[]
                # for x,y in kp2dX['kp_2d'][0,0].tolist():
                #     cv_keypoints.append(cv2.KeyPoint(x, y,10))
                # #raw_img = np.asarray(batch[0,0])
                # raw_img = raw_img[0].numpy()
                # print('@@@@@@@@',raw_img.shape)
                # print('#####',cv_keypoints)
                # cv2.drawKeypoints(raw_img, cv_keypoints, raw_img, color=(255,0,0))
                # cv2.imwrite(f"/home/ubuntu/gyrus/3D_pose/myimg{count}.jpg",raw_img)
    ...
    # for key in vibe_results[0]:
    #     print(key,'  shape====',vibe_results[1][key].shape)
