...
class VIBELoss2(nn.Module):
    ...
    def forward(
        ...
        real_2d = real_2d #reduce(real_2d)

        

        real_3d = data_3d['kp_3d'] #reduce(data_3d['kp_3d'])
        data_3d_theta = data_3d['theta'] #reduce(data_3d['theta'])
        #print('reduced real shape of 2d j---',real_3d.shape)
        ...
        #print("predicted theta shape -----",pred_theta.shape,'3d j shape--',pred_j3d.shape)
        ...
        #print('reduced pred shape of 3d j---',pred_j2d.shape)
        ...
        # ## Removing missing KPs
        # pred_j3d = torch.cat((pred_j3d[:,:-4] , pred_j3d[:,-3:-2]),1)
        # real_3d = torch.cat((real_3d[:,:-4] ,real_3d[:,-3:-2]),1)
        # print("@@@@@@@@@@@@@xx",pred_j3d.shape)
        ...
        #real_shape, pred_shape = data_3d_theta[:, 75:], pred_theta[:, 75:]
        real_pose, pred_pose = data_3d_theta, pred_theta[:, 3:75]

        #print('inside loss shape check----',real_pose.shape,pred_pose.shape)
        ...
        if pred_theta.shape[0] > 0:
            loss_pose = self.smpl_losses(pred_pose, real_pose)
            #loss_shape = loss_shape * self.e_shape_loss_weight
            loss_pose = loss_pose * self.e_pose_loss_weight
            #loss_dict['loss_shape'] = loss_shape
            loss_dict['loss_pose'] = loss_pose

        gen_loss = Variable(torch.as_tensor(list(loss_dict.values())).sum(), requires_grad = True)
        # gen_loss = torch.stack(torch.as_tensor(list(loss_dict.values()))).sum()
        print('LOSS Contribution====',torch.as_tensor(list(loss_dict.values())))
    ...
    def keypoint_loss(self, pred_keypoints_2d, gt_keypoints_2d, openpose_weight, gt_weight):
        ...
        # conf[:, 25:] *= gt_weight
        # loss = (conf * self.criterion_keypoints(pred_keypoints_2d, gt_keypoints_2d[:, :, :-1])).mean()
        loss = (self.criterion_keypoints(pred_keypoints_2d, gt_keypoints_2d)).mean()
